...
export default function MaintenancePage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOptimisticItems((cur) => {
        ...
        if (
          cur.some(
            (it) => getField(it, "id", "Id") === getField(created, "id", "Id")
          )
        )
          return cur;
        return [created, ...cur];
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setOptimisticItems((cur) =>
        cur.filter((it) => getField(it, "id", "Id") !== id)
      );
      await refetch();
    } catch (err) {
  ...
  const serverItems: AnyItem[] = Array.isArray(data)
    ? (data as AnyItem[])
    : (data as any)?.items ?? data ?? [];
  const mergedById = new Map<number | string, AnyItem>();
  (optimisticItems || []).forEach((it) => {
    const id = getField(it, "id", "Id");
    if (id !== undefined) mergedById.set(id, it);
  });
    ...
    <div className="mx-auto w-full max-w-6xl p-6">
      <div className="mb-6 flex items-end justify-between">
        ...
        <div className="flex items-end gap-2">
          ...
          <Button
            onClick={() => refetch()}
            aria-label="Apply maintenance filters"
          >
            Filter
          </Button>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <RoleGate allow={["Admin", "Manager", "Staff"]}>
          ...
          >
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="flex flex-col">
                  ...
                  <div className="text-xs text-gray-600 mt-1">
                    Selected: #{form.propertyId}
                  </div>
                </div>

                <div className="flex flex-col">
                  <label
                    htmlFor="mnt-unit-select"
                    className="text-sm font-medium"
                  >
                    Unit (by Property)
                  </label>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="flex flex-col">
                <label htmlFor="mnt-title" className="text-sm font-medium">
                  Title
                </label>
                <input
                  id="mnt-title"
              ...
              <div className="flex flex-col">
                <label htmlFor="mnt-priority" className="text-sm font-medium">
                  Priority
                </label>
                <select
                  id="mnt-priority"
              ...
              <div className="col-span-1 sm:col-span-2 flex flex-col">
                <label htmlFor="mnt-desc" className="text-sm font-medium">
                  Description
                </label>
                <textarea
                  id="mnt-desc"
          ...
          {isLoading ? (
            ...
            <ul className="divide-y">
              {items.map((m: AnyItem) => {
                ...
                const createdAt =
                  getField<string>(m, "createdAtUtc", "CreatedAtUtc") ??
                  getField<string>(m, "createdAt", "CreatedAt");
                return (
                  <li
                    key={id}
                    ...
                    <div>
                      ...
                      <RoleGate allow={["Admin", "Manager"]}>
                        <div className="mt-2 flex gap-2">
                          <button
                            aria-label={`Mark request ${id} in progress`}
                            onClick={() =>
                              id !== undefined && handleMark(id, 2, m)
                            }
                          ...
                          <button
                            aria-label={`Mark request ${id} resolved`}
                            onClick={() =>
                              id !== undefined && handleMark(id, 4, m)
                            }
