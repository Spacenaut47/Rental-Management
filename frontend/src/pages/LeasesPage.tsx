...
export default function LeasesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const created = await createLease(form).unwrap();

      // add to optimistic list if not present
              ...
              {leases.map((l: any) => (
                <li key={l.id} className="flex items-center justify-between gap-4 py-2">
                  ...
                  <RoleGate allow={["Admin"]}>
                    <button aria-label={`Delete lease ${l.id}`} className="rounded-md px-3 py-1 text-sm text-red-600 hover:bg-red-50" onClick={() => handleDelete(l.id)}>Delete</button>
                  </RoleGate>
                </li>
              ))}
            </ul>
